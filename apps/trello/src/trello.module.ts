  ...
  imports: [
    ConfigModule.forRoot({
      ...
      validationSchema: Joi.object({
        ...
        TRELLO_HOST: Joi.string().required(),
        TRELLO_TCP_PORT: Joi.number().integer().min(1).max(65535).required(),
        TRELLO_HTTP_PORT: Joi.number().integer().min(1).max(65535).required(),
      }),
    }),
    HttpModule,
  ],
